...
public class TicketCreateButton : InteractionModuleBase<SocketInteractionContext>
{
    ...
    {
        if (!await DatabaseManager.TicketExists(Context.User.Id))
        {
            var embed = await CreateEmbed(Context.Client);
            ComponentBuilder buttons = new();
            ModalBuilder modal = null;

            switch ((Context.Interaction as SocketMessageComponent).Data.Values.ElementAt(0))
            {
                case "no_connect":
            ...
            buttons.AddRow(await CreateActionRow());

            if (!Context.Interaction.HasResponded)
            {
                await CreateChannelAsync(Context.Guild, Context.User, $"{Context.User.Mention} your ticket is created here:", embed.Build(), buttons.Build());
                await RespondAsync("You Ticket is created!", ephemeral: true);
            }
        }
        else
        {
            await RespondAsync("There is already an open Ticket! Please use your ticket to get support.", ephemeral: true);
        }
    }
    ...
    {
        var channel = await guild.CreateTextChannelAsync($"ticket-{user.Username}", (c) =>
        {
            c.CategoryId = ConfigManager.GlobalConfig.Channels.TicketCategoryId;
            var overwrites = guild.GetCategoryChannel((ulong)c.CategoryId.Value).PermissionOverwrites.ToList();
