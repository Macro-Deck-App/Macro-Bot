...
public class Program {
	...
	public async Task MainAsync (string[] args) {
		...
		var timer2 = new System.Timers.Timer(25 * 1000);
	...
	private async void Timer_Elapsed (object? sender, ElapsedEventArgs e) {
		...
		if (now.Hour == 5) {
			foreach (var ticket in await DatabaseManager.GetTickets()) {
				try {
					var days = (int)now.Subtract(ticket.Modified).TotalDays;
					if (days > 30) {
						...
						var user = await _client.GetUserAsync(ticket.Author) as SocketUser;
	...
	private async Task MessageReceived (SocketMessage message) {
		if (message.Author is not SocketGuildUser member || member.IsBot || member.Roles.Contains(member.Guild.GetRole(ConfigManager.GlobalConfig.Roles.ModeratorRoleId)))
			return;
		var imageChannels = ConfigManager.GlobalConfig.Channels.ImageOnlyChannels;
	...
	private Task Log (LogMessage logMessage) {
		...
		var module = logMessage.Source switch {
	...
	private async Task<int> UpdateMemberCount () {
		var guild = _client.GetGuild(ConfigManager.GlobalConfig.TestGuildId);
		var memberCount = 0;
		...
		var channel = guild.GetChannel(ConfigManager.GlobalConfig.Channels.MemberScreeningChannelId);
