...
public class ReportingCommands : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task ReportMessage(IUserMessage message)
    {
        ...
        if (assignable != null) createReportAssignments.Remove(assignable);
        ...
        if ((message.Author as IGuildUser).RoleIds.Contains(_botConfig.Roles.AdministratorRoleId) ||
            (message.Author as IGuildUser).RoleIds.Contains(_botConfig.Roles.ModeratorRoleId) ||
            message.Author.IsBot || message.Author.IsWebhook)
        {
            EmbedBuilder embed = new EmbedBuilder()
            {
                Title = "Cannot report message!",
                Description = "It looks like the message is from an admin, a moderator, or a bot.",
                Color = Color.Red
            };
    ...
    public async Task ReportUser(IGuildUser user)
    {
        ...
        if (user.RoleIds.Contains(_botConfig.Roles.AdministratorRoleId) ||
            user.RoleIds.Contains(_botConfig.Roles.ModeratorRoleId) ||
            user.IsBot || user.IsWebhook)
        {
            EmbedBuilder embed = new EmbedBuilder()
            {
                Title = "Cannot report user!",
                Description = "It looks like the user is an admin, a moderator, or a bot.",
                Color = Color.Red
            };
    ...
    public async Task ReportList()
    {
        ...
        {
            str += $"[{report.Id}] > {report.User} by {report.Reporter} {(report.Message is not null ? $"on {report.Channel}/{report.Message}" : "")}\r\n";
        }
