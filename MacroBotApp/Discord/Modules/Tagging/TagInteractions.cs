...
public class TaggingInteractions : InteractionModuleBase<SocketInteractionContext>
{
	...
	{
		var assignable =
			TaggingCommands.createTagAssignments.Find(x =>
	...
	{
		var assignable =
			TaggingCommands.editTagAssignments.Find(x => x.guildId == Context.Guild.Id && x.userId == Context.User.Id);
		if (assignable != null)
		{
			await using var scope = _serviceScopeFactory.CreateAsyncScope();
			var tagRepository = scope.ServiceProvider.GetRequiredService<ITagRepository>();
	...
	{
		var assignable =
			TaggingCommands.deleteTagAssignments.Find(x =>
				x.guildId == Context.Guild.Id && x.userId == Context.User.Id);
		if (assignable != null)
		{
			await using var scope = _serviceScopeFactory.CreateAsyncScope();
...
public class TagEditModal : IModal
{
	[InputLabel("Tag Content")]
	[ModalTextInput("tag_create_content", TextInputStyle.Paragraph, minLength: 1, maxLength: 4000,
		placeholder: "Sadly we cannot provide the existing tag content here automatically due to Discord limitations")]
	public string TagContent { get; set; }
