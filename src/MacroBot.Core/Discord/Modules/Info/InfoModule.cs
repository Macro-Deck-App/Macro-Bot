...
public class BotInfoModule : InteractionModuleBase<SocketInteractionContext>
{
        ...
        if (user.IsBot != true || asUser)
        {
            ...
            {  
                foreach (var activities in user.Activities)
                {
                    ...
                    {
                        acts += $"[Spotify] [**{spot.TrackTitle}**]({spot.TrackUrl}) {string.Join((string?)", ", (IEnumerable<string?>)spot.Artists)} ({spot.Elapsed.Value.ToString(@"hh\:mm\:ss")} / {spot.Duration.Value.ToString(@"hh\:mm\:ss")})\r\n";
                    }
            ...
            embed.AddField("Roles", string.Join((string?)", ", (IEnumerable<string?>)Context.Guild.GetUser(user.Id).Roles.Select(r => r.Mention)), true);
    ...
    public async Task GuildInfo()
    {
        ...
        {
            member = string.Join((string?)", ", (IEnumerable<string?>)Context.Guild.Users.Select(u => u.Mention));
            epm = false;
        }
        ...
        {
            text = string.Join((string?)", ", (IEnumerable<string?>)Context.Guild.TextChannels.Select(u => u.Mention));
        }
        embed.AddField("Text Channels", string.Join((string?)", ", (IEnumerable<string?>)Context.Guild.TextChannels.Select(u => u.Mention)), true);
        string voice;
        ...
        {
            voice = string.Join((string?)", ", (IEnumerable<string?>)Context.Guild.VoiceChannels.Select(u => u.Mention));
        }
