...
public class TaggingInteractions : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task TagDeleteConfirm()
    {
        ...
        if (assignable != null)
        {
            ...
            if (Context.Interaction is not SocketMessageComponent socketMessageComponent)
            {
                return;
            }
            
            await socketMessageComponent.Message.ModifyAsync(msg => msg.Components = new ComponentBuilder().Build());
