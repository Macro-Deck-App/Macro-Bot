...
namespace MacroBot.Core.Discord;
...
public static class DiscordStatusCheckMessageBuilder
{
    ...
    public static Embed Build(StatusCheckResult[] statusCheckResults)
    {
        ...
        var nowOffline = statusCheckResults.Where(x => x.StateChanged && x is { Online: false, OnlineWithWarnings: false }).ToArray();
        var nowOnline = statusCheckResults.Where(x => x.StateChanged && x is { Online: true, OnlineWithWarnings: false }).ToArray();
        var nowOnlineWithWarnings = statusCheckResults.Where(x => x.StateChanged && x is { Online: true, OnlineWithWarnings: true }).ToArray();
